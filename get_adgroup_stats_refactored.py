...
# -*- coding: utf-8 -*-
"""
Скрипт для получения статистики по группам объявлений из API отчетов Яндекс.Директ
...
class AdGroupStatsProcessor:
    """Обработчик для получения статистики по группам объявлений"""
    ...
    def process_reports(self):
        ...
        print("🚀 Запуск получения статистики по группам объявлений")
        print("="*60)
        ...
        try:
            ...
            for account in accounts:
                ...
                if self.api_client.test_connection():
                    ...
                    stats_data = self.get_adgroup_stats(campaign_ids, start_date, end_date, deleted_group_ids)
                    if stats_data:
                        print("✅ Статистика по группам объявлений получена успешно")
                        self.current_account = account
                        break
                    else:
                        print("❌ Не удалось получить статистику по группам объявлений")
                else:
            ...
            if stats_data:
                # Сохраняем данные
                self.save_adgroup_stats(stats_data, report)
            else:
    ...
    def get_adgroup_stats(self, campaign_ids: List[int], start_date: str, end_date: str, deleted_group_ids: List[int] = None) -> Optional[Dict]:
        """Получает статистику по группам объявлений через Reports API"""
        try:
            # Используем метод из api_client для создания отчета
            report_data = self.api_client.create_adgroup_performance_report(
            ...
            print("🔍 Отчет по группам объявлений создан, ожидаем обработки...")
            
            # Ждем обработки отчета
            processed_data = self.wait_for_report_processing(report_data)
            
            if processed_data:
                print("✅ Отчет по группам объявлений обработан успешно")
                return processed_data
            else:
                print("❌ Ошибка обработки отчета по группам объявлений")
                return None
    ...
    def wait_for_report_processing(self, report_data: Dict, max_wait_time: int = 300) -> Optional[Dict]:
        ...
        # Этот метод больше не нужен, так как create_adgroup_performance_report
        # сам обрабатывает цикл ожидания согласно официальному примеру
        return report_data
    
    def save_adgroup_stats(self, stats_data: Dict, report: Dict):
        """Сохраняет статистику групп объявлений в MinIO"""
        try:
            # Выводим сводку
            self.display_stats_summary(stats_data)
            
            # Сохраняем данные в MinIO
            success = self.minio_client.upload_adgroup_stats_data(stats_data, report['id'])
            if success:
            ...
            print(f"❌ Ошибка сохранения статистики групп объявлений: {e}")
    
    def display_stats_summary(self, stats_data: Dict):
        """Выводит краткую сводку по статистике групп объявлений"""
        try:
            print(f"\n📊 Сводка по статистике групп объявлений:")
                        ...
                        if data_lines:
                            ...
                            for line in data_lines:
                                ...
                                if len(fields) >= 11: # CampaignId, AdGroupId, AdGroupName, CampaignType, AdNetworkType, Impressions, Clicks, Ctr, BounceRate, Cost, AvgCpc
                                    ...
                                    adgroup_id = fields[1] if fields[1] else 'N/A'
                                    adgroup_name = fields[2] if fields[2] else 'N/A'
                                    campaign_type = fields[3] if fields[3] else 'N/A'
                                    ad_network_type = fields[4] if fields[4] else 'N/A'
                                    impressions = int(fields[5]) if fields[5].isdigit() else 0
                                    clicks = int(fields[6]) if fields[6].isdigit() else 0
                                    ctr = float(fields[7]) if fields[7].replace('.', '').isdigit() else 0
                                    bounce_rate = float(fields[8]) if fields[8].replace('.', '').isdigit() else 0
                                    cost = float(fields[9]) if fields[9].replace('.', '').isdigit() else 0
                                    avg_cpc = float(fields[10]) if fields[10].replace('.', '').isdigit() else 0
                                    
                                    print(f"   {shown_adgroups + 1}. Группа объявлений ID: {adgroup_id}")
                                    print(f"      Название: {adgroup_name}")
                                    print(f"      Кампания ID: {campaign_id}, Тип кампании: {campaign_type}")
                                    print(f"      Тип сети: {ad_network_type}")
                            ...
                            if total_clicks > 0:
                                overall_avg_cpc = total_cost / total_clicks
                                print(f"   Общая средняя стоимость клика: {overall_avg_cpc:.2f} руб.")
                        else:
                            print("   Нет данных в отчете")
                        ...
                        if 'result' in report_data and 'rows' in report_data['result']:
                            ...
                            if rows:
                                ...
                                for row in rows:
                                    ...
                                    adgroup_id = row.get('AdGroupId', 'N/A')
...
def main():
    """Основная функция"""
    print("🚀 Запуск скрипта получения статистики по группам объявлений")
    print("="*60)
    ...
    processor = AdGroupStatsProcessor()
